...
export default (state = expensesReducerDefaultState, action) => {
    switch (action.type) {
        case 'ADD_EXPENSE':         ///remember that you don't want to change the array so don't use Push
            return [                /// state.concat(action.expense);  we are going to use the spread operator 
                ...state,
                action.expense
            ];
        case 'REMOVE_EXPENSE':
        ...
        case 'EDIT_EXPENSE':
            return state.map((expense)=> { //now we are going to map which allows us to go through every single item and do some conditional logic to change some of them. and pass along the updator function {}
                if (expense.id === action.id) { //expense id is the current item in the array it is checking and action.id the one we are changing
                    return {
